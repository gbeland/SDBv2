                ...
                using (var cmd = conn.CreateCommand())
                {
                    ...
                    cmd.CommandText =
                        $@"INSERT INTO magicinfo_asset 
                                   (asset_name, alias, firmware, ip, model, mac_address, serial, ignore_alert, address_fky, customer_fky, market_fky, server_fky)
                            VALUES (@asset_name, @alias, @firmware, @ip, @model, @mac_address, @serial, @ignore_alert, @address_fky, @customer_fky, @market_fky, @server_fky);";
