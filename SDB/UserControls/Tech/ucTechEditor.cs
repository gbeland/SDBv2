...
namespace SDB.UserControls.Tech
{
	public partial class TechEditor : UserControl
	{
        ...
        public event TechEvent OpenURL_LatLongMap;
        #endregion
		...
		private void Populate()
		{
            ...
            if(_tech.Longitude == null)
            {
                UpdateGeoCode();
                UpdateTechObjectFromFields();
                if(_tech.Longitude != 0)
		...
		{
            txtLatitude.Text = _tech.Latitude.ToString();
            txtLongitude.Text = _tech.Longitude.ToString();
		...
		private void UpdateTechObjectFromFields()
		{
            ...
            _tech.Latitude = latitude;
            _tech.Longitude = longitude;
        ...
        {
            OpenURL_LatLongMap?.Invoke(_tech);
        }
        ...
        private void UpdateGeoCode()
        {
            ...
            var techLocation = string.Format("{0}, {1}, {2} {3}", address, txtCity.Text, txtState.Text, txtCountry.Text);
            try
            {
                geoInfo = ClassGeoCode.GetAddressV3(techLocation);
            }
            catch (Exception exc)
            {
                ClassLogFile.AppendToLog(exc);
            ...
            //{
            //    geoCodeConfirmForm.AssetLocation = techLocation;
