    ...
    public class ClassTech
    {
		...
		private static ClassTech GetFromReader(MySqlDataReader reader)
		{
            ...
            return new ClassTech
            {
                ...
                Latitude = reader.GetDBDouble_Null("latitude"),
                Longitude = reader.GetDBDouble_Null("longitude"),
		...
		public static void Update(ClassTech tech)
		{
			using (var conn = ClassDatabase.CreateMySqlConnection())
			{
				...
				using (var cmd = conn.CreateCommand())
				{
					cmd.CommandText =
						@"UPDATE techs SET
						tech = @Tech, latitude = @latitude, longitude = @longitude, address = @Address, city = @City, state = @State, zip = @Zip, country = @Country,
                    ...
                    cmd.Parameters.AddWithValue("longitude", tech.Longitude);
                    cmd.Parameters.AddWithValue("latitude", tech.Latitude);
		...
		public static void AddNew(ref ClassTech tech)
		{
			using (var conn = ClassDatabase.CreateMySqlConnection())
			{
				...
				using (var cmd = conn.CreateCommand())
				{
					cmd.CommandText =
						@"INSERT INTO techs
							(tech, latitude, longitude, address, city, state, zip, country,
							shiptomailing,
