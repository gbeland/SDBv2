...
namespace SDB.Classes.RMA
{
	...
	public partial class ClassRMA : ISupportsJournal
	{
		...
		public bool IsHot { get; set; } //SO_HOT_RIGHT_NOW.jgp
		...
		private const string _RMA_BASE_QUERY =
			@"SELECT
				...
				r.job_number, r.po_number, r.legacy_rma, r.notes, r.flag_apr, r.flag_hot, r.hot_dt, r.flag_accounting,
				...
				using (var cmd = conn.CreateCommand())
				{
					cmd.CommandText =
                        @"INSERT INTO rma
							...
							flag_apr, flag_hot, hot_dt, flag_computer)
						VALUES
							(@TicketID, @AssetID, @TechID,
							NOW(), @CreationUserID,
							@JobNumber, @PONumber, @LegacyRMA, @Notes,
							@IsAPR, @IsHot, @hot_dt, @HasComputer);";
                    ...
                    cmd.Parameters.AddWithValue("hot_dt", newRMA.Hot_Date);
		...
		public static void Update(ClassRMA rma)
		{
			using (var conn = ClassDatabase.CreateMySqlConnection())
			{
				...
				using (var cmd = conn.CreateCommand())
				{
                    ...
                    cmd.Parameters.AddWithValue("hot_dt", rma.Hot_Date);
		...
		private static ClassRMA GetFromReader(MySqlDataReader reader)
		{
            ...
            rma.Hot_Date = reader.GetDBDateTime_Null("hot_dt");
