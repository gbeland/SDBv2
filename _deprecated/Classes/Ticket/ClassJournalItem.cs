...
namespace SDB.Classes
{
	...
	public class ClassJournalItem
	{
		...
		{
			get { return ExpireDateTime.HasValue && DateTime.Now > ExpireDateTime; }
		}
		...
		/// </summary>
		[UsedImplicitly]
		public DateTime SoftExpireDateTime
		{
			get { return ExpireDateTime.HasValue ? ExpireDateTime.Value : JournalDateTime.AddHours(24); }
		}
		...
		public static void PopulateLastEntriesForTicketList(List<ClassTicket> tickets)
		{
			...
			using (var conn = ClassDatabase.CreateMySqlConnection())
			{
				...
				using (var cmd = conn.CreateCommand())
				{
					cmd.CommandText = string.Format(
						...
						AND tj.ticket_id in ({0});", string.Join(",", parentIds));
			...
			foreach (var ticket in tickets)
				ticket.Journal.Entries.Add(entries.Single(e => e.Parent_ID == ticket.ID));
		}
