...
namespace SDB.UserControls
{
	...
	public partial class CustomerEditor : UserControl
	{
		...
		private void Market_Email_DoubleClick(object sender, EventArgs e)
		{
			...
			Process.Start(string.Format("mailto:{0}?subject={1}", _market.Email_Addresses, SUBJECT));
		}
		...
		private void olvTickets_MouseDoubleClick(object sender, MouseEventArgs e)
		{
			...
			var selectedTicket = (ClassTicket_Multi)olvTickets.SelectedObject;
		...
		{
			using (var marketSelect = new FormMarket_Select())
			{
		...
		{
			ucAssetList1.SetObjects(ClassAsset.GetByMarket(_market.ID).ToList());
		}
		...
		private void PopulateTickets()
		{
			...
			{
				var openTickets = ClassTicket_Multi.GetTickets_Open(_market).ToList();
			...
			else
			{
				...
				var closedTickets = ClassTicket_Multi.GetClosedTickets_ByMarket_SinceDate(_market, aYearAgo);
		...
		private void PopulateMarketInformation()
		{
			if (_market != null)
			{
				...
				{
					ClassSalesperson sp = ClassSalesperson.GetByID((int)_market.SalespersonID);
		...
		private void UpdateClassesFromFields()
		{
			...
			var sp = ClassSalesperson.GetSalespeople().FirstOrDefault(Fullname => Fullname.FullName == cmbCustomerSalesperson.Text);
			if (sp != null)
