...
namespace SDB.UserControls.Ticket
{
	public partial class TicketEditor : UserControl
	{
		...
		private List<ClassLegacyRMA> _legacyRmas;
		...
		private void Populate_LegacyRMA()
		{
			...
			_legacyRmas = ClassLegacyRMA.GetByTicket(_ticket.ID).ToList();
			olvLegacyRMAs.SetObjects(_legacyRmas);

			if (_legacyRmas == null || _legacyRmas.Count == 0)
				txtRMAQty.Text = "0";
			else
				txtRMAQty.Text = _legacyRmas.Count.ToString(CultureInfo.InvariantCulture);
		}
		...
		{
			ViewLegacyRMA?.Invoke(((ClassLegacyRMA)olvLegacyRMAs.SelectedObject).ID);
		}
