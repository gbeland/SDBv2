...
namespace SDB.Forms.General
{
	...
	{
		#region Globals
		private readonly FormLegacyRMAManagement _legacyRMAForm = new FormLegacyRMAManagement();
		...
		public FormMain()
		{
            ...
            //NEW VERSION
            //_childForms = new[] { (Form)_legacyRMAForm, _formRMA, _rmaEditorForm, _shipmentListForm, _shipmentEditorForm, _reportingForm, _notificationsForm, _cameraCheckReviewForm };
            //OLD VERSION
            _childForms = new[] { (Form)_legacyRMAForm, _rmaListForm, _rmaEditorForm, _shipmentListForm, _shipmentEditorForm, _reportingForm, _notificationsForm, _cameraCheckReviewForm };
			...
			_legacyRMAForm.TicketDoubleClicked += LegacyRMAForm_TicketDoubleClicked;
			_legacyRMAForm.CreateShipment += LegacyRMAForm_CreateShipment;
			...
			_rmaEditorForm.ViewLegacyRMA += LegacyRMA_Load;
		...
		private void FormMain_Load(object sender, EventArgs e)
		{
            ...
            if (string.IsNullOrEmpty(GS.Settings.DBPassword))
			{
				MessageBox.Show("The database password is not specified in settings. If this is a new installation, please contact IT or your supervisor for help.", "Blank Database Password", MessageBoxButtons.OK, MessageBoxIcon.Warning);
			}
#endif
...
#if DEBUG
			//This should change 
			//GrantLogin(ClassUser.GetByID(9120)); //JaredY Logon
            GrantLogin(ClassUser.GetByID(90674)); //TylerA Logon
            SetupUIForLogOn();
#endif
		}
		...
		private void mnuRma_Legacy_Click(object sender, EventArgs e)
		{
			LegacyRMA_Search();
		}
		#endregion
		...
		private void ucTicketEditor1_ViewOSA(ClassTicket ticket, ClassAsset asset, ClassTech tech)
		{
            if (asset.IsPMC)
            {
                if (ClassEmailGenerator.GenerateDocument_PMC(ticket, asset, tech))
                    OpenFile(Path.Combine(Path.GetTempPath(), $"{ticket.ID}_PMC.doc"));
            }
            else
		...
		private void ucTicketEditor1_ViewLegacyRMA(int rmaID)
		{
			LegacyRMA_Load(rmaID);
		}
		...
		private void ucAssetEditor1_ViewLegacyRMA(int rmaID)
		{
			LegacyRMA_Load(rmaID);
		}
		...
		private void ucTechEditor1_ViewLegacyRMA(int rmaID)
		{
			LegacyRMA_Load(rmaID);
		}
		...
		#region Legacy RMA
		private void LegacyRMA_Search()
		{
			LegacyRMA_ShowForm();
			_legacyRMAForm.RMA_StartSearch();
		}

		private void LegacyRMA_ShowForm()
		{
			_legacyRMAForm.Show();
			if (_legacyRMAForm.WindowState == FormWindowState.Minimized)
				_legacyRMAForm.WindowState = FormWindowState.Normal;
			_legacyRMAForm.BringToFront();
		}

		private void LegacyRMA_Load(int rmaID)
		{
			LegacyRMA_ShowForm();
			if (_legacyRMAForm.WindowState == FormWindowState.Minimized)
				_legacyRMAForm.WindowState = FormWindowState.Normal;
			_legacyRMAForm.RMA_Load(rmaID);
		}

		private void LegacyRMAForm_TicketDoubleClicked(int ticketID)
		{
			Ticket_Load(ticketID);
		}

		private void LegacyRMAForm_CreateShipment(ClassLegacyRMA rma)
		{
			_shipmentEditorForm.CreateShipmentForLegacyRMA(rma);
			ShipmentEditor_ShowForm();
		}
		#endregion
