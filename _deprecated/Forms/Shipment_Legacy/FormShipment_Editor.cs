	...
	public partial class FormShipment_Editor : Form
	{
       ...
       // private String
		...
		private void btnLedger_Add_Click(object sender, EventArgs e)
		{
			...
			using (var shipmentItemForm = new FormShipmentItem())
			{
		...
		private void UpdateOrCreateShipment()
		{
			...
			_shipment.HasComputer = _ledger.Any(i => i.AssemblyType.IsComputer);
			...
			if (_shipment.ID < 1)
			{
				...
				_shipment.IsReady = !_shipment.HasComputer;
				if (_shipment.IsReady)
		...
		private string CreateHtmlForShipment(int shipmentID)
		{
			...
			var sb = new StringBuilder();
			sb.AppendFormat("<html><head><style>th {{ font-weight: bold; }} td {{ border: dotted 1px black; padding: 0 4px 0 4px; }} table {{ font-size: 10pt; }} hr {{ margin: 0px; }} h2,h3 {{ margin-bottom: 0px; }} @media print {{ .pbreak {{ page-break-after: auto }} }}</style><title>Prismview Shipment {0}</title></head>", exportShipment.ID).AppendLine();
		...
		private string CreateHtmlForShipmentLabel(int shipmentID)
		{
			...
			sb.AppendLine("<html>                                                                                         ");
			sb.AppendLine("	<head>                                                                                        ");
			sb.AppendFormat("		<title>Prismview Shipment {0} Label</title>                                           ", shipment.ID).AppendLine();
			...
			sb.AppendLine("				PRISMVIEW, LLC<br />                                                      ");
			sb.AppendLine("				1651 N 1000 W<br />                                                               ");
		...
		private void ShipmentDetail_EditSelectedItem()
		{
			...
			using (var shipmentItemForm = new FormShipmentItem((ClassShipment_Item)olvLedger.SelectedObject))
			{
				if (shipmentItemForm.ShowDialog() != DialogResult.OK)
					return;
		...
		/// <summary>
		/// Creates a new shipment, using the Legacy RMA to specify default Asset, Ticket, and Tech.
		/// </summary>
		public void CreateShipmentForLegacyRMA(ClassLegacyRMA rma)
		{
			_affinityRmaID = rma.ID;
			_affinityAssetID = rma.Asset_ID;
			_affinityTicketID = rma.Ticket_ID;
			Shipment_Create(ClassTech.GetByID(rma.Tech_ID));
		}
